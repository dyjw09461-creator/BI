...
`金币联盟` 和 `报表BI` 使用同一个业务库 `BIDemo_AccumulateCoin`。金币联盟负责产生业务数据，报表BI负责把这些数据抽取到 `BI_GoldCoin_DW` 后展示到 BI 大屏。
...
| 金币联盟操作 | 业务库变化 | 报表BI变化 |
|---|---|---|
| 会员注册 | 新增 `CustomerInfo` 和会员 `Account` | 会员总数、会员增长趋势变化 |
| 商家注册 | 新增 `BusinessMen` 和商家 `Account` | 商家总数、商家贡献统计变化 |
| 新增商家商品 | 新增 `ProductInfo` | 商品积分贡献排行变化 |
| 上传/批量生成积分码 | 新增 `JFCode` | 积分码状态和商品积分码数量变化 |
| 会员积金币 | 更新积分码状态，写入 `AccountTradeLog` | 金币收入、积分流水、商品贡献变化 |
| 会员兑换礼品 | 新增 `OrderInfo`、`OrderGift`，更新库存和账户 | 订单趋势、礼品排行、金币支出变化 |
| 取消/完成订单 | 更新 `OrderInfo` 状态，可能回补账户和库存 | 订单状态和订单统计变化 |

如果在金币联盟中新增了数据，需要执行增量 ETL 后刷新报表：
